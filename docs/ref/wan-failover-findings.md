...
> **หมายเหตุจากทีม AI:** "auto healing" ที่เห็นน่าจะมาจาก static route `0.0.0.0/0
> proto 120 metric 10` ที่พบใน `ip route show default` ของ S-6 (ผ่าน
> `wlx0cef1548ff2b` gw `192.168.1.78`) — ตรงกับกติกา D-2 ข้อ 1 ของแผน (static
> route 0.0.0.0/0 ชนะทุกอย่าง) ต้องเช็คหน้า Static Routes ใน UI ว่ามี entry นี้
> ตั้งใจไว้หรือไม่ ก่อนเริ่มทดสอบ Phase 2 (Task 14-15) เพราะจะบล็อก failover
> ไม่ให้ทำงานตามที่ออกแบบไว้ (พฤติกรรมถูกต้องตามแผน แต่ต้องรู้ตัวก่อนทดสอบ)
...
1. **มี static default route (`0.0.0.0/0 proto 120 metric 10` ผ่าน `wlx0cef1548ff2b`
   gw `192.168.1.78`) อยู่บนบอร์ดแล้วตอนทดสอบ** (เห็นใน `ip route show default`
   ของ S-6) — น่าจะเป็นสาเหตุของ "auto healing" ที่สังเกตเห็นใน S-2 ตรงกับกติกา
   D-2 ข้อ 1 ของแผน (static route ชนะทุกอย่าง) **ต้องตรวจสอบหน้า Static Routes
   ใน UI ก่อนเริ่มทดสอบ Phase 2 (Task 14-15)** ว่า entry นี้ตั้งใจไว้หรือไม่ —
   ถ้ายังอยู่ตอนทดสอบ failover controller จะ "ยอมแพ้" ไม่สลับ WAN ให้ตามที่
   ออกแบบไว้ (เป็นพฤติกรรมที่ถูกต้อง แต่ทำให้ทดสอบไม่เห็นผลถ้าไม่รู้ตัวก่อน)
2. **S-3/S-4/S-5 ยังไม่มีหลักฐานดิบครบตามที่ checklist ขอ** ไม่กระทบการเริ่ม
